...
def construct_buffer(config, max_blocks):
    ...
    if n_blocks > 16:
        print("ERROR: buffer would require {} blocks" +
                "which is more than allowed (16)".format(n_blocks))
        return None

    if n_blocks > max_blocks:
        print("ERROR: buffer would require {} blocks" +
                "which is more than max_blocks ({})".format(n_blocks, max_blocks))
        return None

    if n_blocks > 5:
        print("Warning: buffer would require {} blocks" +
                "which is more than some BMCs support".format(n_blocks))
